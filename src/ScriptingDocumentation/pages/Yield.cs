...
/// var map = HeightMap.Noise();
/// yield map;
/// yield HeightMap.BumpMap(map) as "BumpMap";
/// @endcode
/// 		
/// The primary map must always be yieled during execution of the script, otherwise an runtime error will be triggered when the execution finishes. Each map can be only yielded once, otherwise an runtime error is triggered.
/// 
