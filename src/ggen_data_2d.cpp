...
namespace GeoGen {
	...
	GGen_Data_2D::GGen_Data_2D(GGen_Size width, GGen_Size height, GGen_Height value)
	{
		...
		GGen_Script_Assert(width < Generator::GetMaxMapSize());
		GGen_Script_Assert(height < Generator::GetMaxMapSize());

		GGen_Script_Assert(GGen_Data_2D::num_instances < Generator::GetMaxMapCount());
	...
	GGen_Height GGen_Data_2D::GetValueInterpolated(GGen_Coord x, GGen_Coord y, GGen_Size scale_to_width, GGen_Size scale_to_height)
	{
		...
		GGen_Script_Assert(scale_to_width <= Generator::GetMaxMapSize());
		GGen_Script_Assert(scale_to_height >= GGEN_MIN_MAP_SIZE);
		GGen_Script_Assert(scale_to_height <= Generator::GetMaxMapSize());
	...
	void GGen_Data_2D::ScaleTo(GGen_Size new_width, GGen_Size new_height, bool scale_values)
	{
		...
		GGen_Script_Assert(new_width <= Generator::GetMaxMapSize());
		GGen_Script_Assert(new_height >= GGEN_MIN_MAP_SIZE);
		GGen_Script_Assert(new_height <= Generator::GetMaxMapSize());
	...
	void GGen_Data_2D::ResizeCanvas(GGen_Size new_width, GGen_Size new_height, GGen_CoordOffset new_zero_x, GGen_CoordOffset new_zero_y)
	{
		...
		GGen_Script_Assert(new_height >= GGEN_MIN_MAP_SIZE);
		GGen_Script_Assert(new_height <= Generator::GetMaxMapSize());
		GGen_Script_Assert(new_zero_x <= Generator::GetMaxMapSize());
		GGen_Script_Assert(new_zero_x >= -Generator::GetMaxMapSize());
		GGen_Script_Assert(new_zero_y <= Generator::GetMaxMapSize());
		GGen_Script_Assert(new_zero_y >= -Generator::GetMaxMapSize());
	...
	void GGen_Data_2D::Noise(GGen_Size minFeatureSize, GGen_Size maxFeatureSize, GGen_Amplitudes* amplitudes)
	{
		...
		GGen_Script_Assert(minFeatureSize <= Generator::GetMaxMapSize());
		GGen_Script_Assert(maxFeatureSize > 0);
		GGen_Script_Assert(maxFeatureSize <= Generator::GetMaxMapSize());
	...
	void GGen_Data_2D::ReturnAs(const GGen_String &name)
	{
		if(Generator::GetInstance()->return_callback == NULL) {
			Generator::GetInstance()->ThrowMessage(GGen_Const_String("The script returned a named map, but return handler was not defined"), GGEN_WARNING);
			return;
		}
