...
namespace GeoGen {
	uint16 Data_2D::num_instances = 0;
	set<Data_2D*> Data_2D::instances;

	Data_2D::Data_2D(Size width, Size height, Height value)
	{
		ScriptAssert(Generator::GetInstance()->GetStatus() != GGEN_LOADING_MAP_INFO);

		ScriptAssert(width > 1 && height > 1);
		ScriptAssert(width < Generator::GetMaxMapSize());
		ScriptAssert(height < Generator::GetMaxMapSize());

		ScriptAssert(Data_2D::num_instances < Generator::GetMaxMapCount());
		Data_2D::num_instances++;

		Data_2D::instances.insert(this);
	...
	Data_2D* Data_2D::Clone()
	{
		Data_2D* victim = new Data_2D(this->width, this->height, 0);
	
		/* Copy the data */
		memcpy(victim->data, this->data, sizeof(Height) * this->length);
	...
	Data_2D::~Data_2D()
	{
		assert(Data_2D::instances.find(this) != Data_2D::instances.end());

		Data_2D::num_instances--;
		Data_2D::instances.erase(this);
		delete [] this->data;
	}

	Size Data_2D::GetWidth()
	{
		return this->width;
	}

	Size Data_2D::GetHeight()
	{
		return this->height;
	}

	Index Data_2D::GetLength()
	{
		return this->length;
	}

	Distance Data_2D::GetMaxDistance(){
		return (Distance) sqrt((double) this->width * (double) this->height);
	}

	Height Data_2D::GetValue(Coord x, Coord y)
	{
		ScriptAssert(x < this->width && y < this->height);
	
		return this->data[x + this->width * y];
	}

	Height Data_2D::GetValueInterpolated(Coord x, Coord y, Size scale_to_width, Size scale_to_height)
	{
		ScriptAssert(y < scale_to_height)
		ScriptAssert(x < scale_to_width);
		ScriptAssert(scale_to_width >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(scale_to_width <= Generator::GetMaxMapSize());
		ScriptAssert(scale_to_height >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(scale_to_height <= Generator::GetMaxMapSize());
		...
		Height value_y_left, value_y_right;	
		...
		Coord base_x = scale_to_width > this->width ? (Coord) floor((double)x / ratio_x) : (Coord) floor((double)x / ratio_x + 0.5);
		Coord base_y = scale_to_height > this->height ? (Coord) floor((double)y / ratio_y) : (Coord) floor((double)y / ratio_y + 0.5);
		...
		if (scale_to_height > this->height) {
			if (remainder_y == 0) {
				...
				value_y_left = (Height) ((double) this->data[(Coord) base_x + this->width * base_y] * (1 - remainder_y) + (double) this->data[base_x + this->width * (base_y + 1)] * (remainder_y));
				value_y_right = (Height) ((double) this->data[(Coord) base_x + 1 + this->width * base_y] * (1 - remainder_y) + (double)this-> data[base_x + 1 + this->width * (base_y + 1)] * (remainder_y));
			}
		} else {
			value_y_left = value_y_right = (Height) this->data[(Coord) base_x + this->width * base_y];
		}
		...
		if (scale_to_width > this->width) {
			...
			return (Height) ((double) value_y_left * (1 - remainder_x) + (double) value_y_right * (remainder_x));
		} else {
			return ((Height) this->data[(Coord) base_x + width * base_y] + value_y_left) /2;
		}
	}

	void Data_2D::SetValue(Coord x, Coord y, Height value)
	{
		ScriptAssert(x < this->width && y < this->height);
	...
	void Data_2D::SetValueInRect(Coord x1, Coord y1, Coord x2, Coord y2, Height value)
	{
		ScriptAssert(x2 < this->width && y2 < this->height);
		ScriptAssert(x1 <= x2 && y1 <= y2);
	...
	void Data_2D::ScaleTo(Size new_width, Size new_height, bool scale_values)
	{
		ScriptAssert(new_width >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(new_width <= Generator::GetMaxMapSize());
		ScriptAssert(new_height >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(new_height <= Generator::GetMaxMapSize());
		...
		Height* new_data = new Height[new_width * new_height];

		ScriptAssert(new_data != NULL);

		/* Fill the new array */
		for (Coord y = 0; y < new_height; y++) {
			for (Coord x = 0; x < new_width; x++) {
				new_data[x + y * new_width] = scale_values ? (Height) ((double) this->GetValueInterpolated(x , y, new_width, new_height) * ratio) : this->GetValueInterpolated(x , y, new_width, new_height);
			}
		}
	...
	void Data_2D::ScaleValuesTo(Height new_min, Height new_max)
	{
		ScriptAssert(new_max > new_min);

		ExtHeight min = this->Min();
		ExtHeight max = this->Max() - min;

		ExtHeight ext_new_min = new_min;
		ExtHeight ext_new_max = (ExtHeight) new_max - ext_new_min;

		if (max == 0) return;

		for (Index i = 0; i < this->length; i++) {
			this->data[i] = (Height) (ext_new_min + ((ExtHeight) this->data[i] - min) * ext_new_max / max);
		}
	}

	void Data_2D::Scale(double ratio, bool scale_values)
	{
		this->ScaleTo((Size) ((Size) (double) this->width * ratio), (Size) ((double) this->height * ratio), scale_values);
	}

	void Data_2D::ResizeCanvas(Size new_width, Size new_height, CoordOffset new_zero_x, CoordOffset new_zero_y)
	{
		ScriptAssert(new_width >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(new_width <= Generator::GetMaxMapSize());
		ScriptAssert(new_height >= GGEN_MIN_MAP_SIZE);
		ScriptAssert(new_height <= Generator::GetMaxMapSize());
		ScriptAssert(new_zero_x <= Generator::GetMaxMapSize());
		ScriptAssert(new_zero_x >= -Generator::GetMaxMapSize());
		ScriptAssert(new_zero_y <= Generator::GetMaxMapSize());
		ScriptAssert(new_zero_y >= -Generator::GetMaxMapSize());
	...
	void Data_2D::Fill(Height value)
	{
		for (Index i = 0; i < this->length; i++) {
			this->data[i] = value;
		}
	...
	void Data_2D::FillMasked(Height value, Data_2D* mask, bool relative)
	{
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++)	{
				Height maskValue = mask->GetValueInterpolated(x, y, this->width, this->height);
				this->data[x + y * width] = maskValue * value + (max - maskValue) * this->data[x + y * width];
			}
		}	
	}

	void Data_2D::Add(Height value)
	{
		for (Index i = 0; i < this->length; i++) {
			this->data[i] += value;
		}
	...
	void Data_2D::AddMap(Data_2D* addend)
	{
	...
	void Data_2D::ReplaceValue(Height needle, Height replace)
	{
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++)	{
	...
	void Data_2D::AddTo(Data_2D* addend, CoordOffset offset_x, CoordOffset offset_y)
	{
		ScriptAssert(addend != NULL);
		ScriptAssert(offset_x > -this->width);
		ScriptAssert(offset_x < this->width);
		ScriptAssert(offset_y > -this->height);
		ScriptAssert(offset_y < this->height);

		/* Walk through the items where the array and the addend with offset intersect */
		for (Coord y = MAX(0, offset_y); y < MIN(this->height, offset_y + addend->height); y++) {
			for (Coord x = MAX(0, offset_x); x < MIN(this->width, offset_x + addend->width); x++) {
				this->data[x + y * this->width] += addend->data[(x - offset_x) + (y - offset_y) * addend->width];
			}
		}
	}

	void Data_2D::AddMapMasked(Data_2D* addend, Data_2D* mask, bool relative)
	{
		ScriptAssert(addend != NULL);
		ScriptAssert(mask != NULL);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				this->data[x + y * this->width] += (ExtHeight) addend->GetValueInterpolated(x, y, this->width, this->height) * (ExtHeight) mask->GetValueInterpolated(x, y, this->width, this->height) / max;
			}
		}
	}

	void Data_2D::AddMasked(Height value, Data_2D* mask, bool relative)
	{
		ScriptAssert(mask != NULL);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++)	{
				this->data[x + y * width] += (ExtHeight) value * (ExtHeight) mask->GetValueInterpolated(x, y, this->width, this->height) / max;
			}
		}
	}

	void Data_2D::Multiply(double factor)
	{
		for (Index i = 0; i < this->length; i++) {
			this->data[i] = (Coord) (factor * (double) this->data[i]);
		}
	}


	void Data_2D::MultiplyMap(Data_2D* factor)
	{
	...
	void Data_2D::Clamp(Height min, Height max)
	{
	...
	void Data_2D::CropValues(Height min, Height max)
	{
		ScriptAssert(max > min);
	...
	void Data_2D::Union(Data_2D* victim)
	{
	...
	void Data_2D::UnionTo(Data_2D* victim, CoordOffset offset_x, CoordOffset offset_y)
	{
		ScriptAssert(victim != NULL);
		ScriptAssert(offset_x > -this->width);
		ScriptAssert(offset_x < this->width);
		...
		for (Coord y = MAX(0, offset_y); y < MIN(this->height, offset_y + victim->height); y++) {
			for (Coord x = MAX(0, offset_x); x < MIN(this->width, offset_x + victim->width); x++) {
				this->data[x + y * width] = MAX(victim->data[(x - offset_x) + (y - offset_y) * victim->width], this->data[x + y * this->width]);
			}
		}
	}

	void Data_2D::Intersection(Data_2D* victim)
	{
		ScriptAssert(victim != NULL);
	...
	void Data_2D::IntersectionTo(Data_2D* victim, CoordOffset offset_x, CoordOffset offset_y)
	{
		ScriptAssert(victim != NULL);
		ScriptAssert(offset_x > -this->width);
		ScriptAssert(offset_x < this->width);
	...
	void Data_2D::Combine(Data_2D* victim, Data_2D* mask, bool relative)
	{
		ScriptAssert(victim != NULL);
		ScriptAssert(mask != NULL);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {	
				this->data[x + y * this->width] = 
					(ExtHeight) this->data[x + y * this->width] * (ExtHeight) mask->GetValueInterpolated(x, y, this->width, this->height) / max + 
					(ExtHeight) victim->GetValueInterpolated(x, y, this->width, this->height) * (ExtHeight) (max - mask->GetValueInterpolated(x, y, this->width, this->height))/ max;
			}
		}
	}
	...
	void Data_2D::Project(Data_1D* profile, Direction direction)
	{
		ScriptAssert(profile != NULL);
	...
	Data_1D* Data_2D::GetProfile(Direction direction, Coord coordinate)
	{
		ScriptAssert(direction == GGEN_VERTICAL || coordinate < this->width);
		ScriptAssert(direction == GGEN_HORIZONTAL || coordinate < this->height);
	
		Data_1D* output;
	
		if (direction == GGEN_HORIZONTAL) {
			output = new Data_1D(this->width, 0);
		
			for  (Coord x = 0; x < this->width; x++) {		
				output->data[x] = this->data[x + coordinate * this->width];
			}
		} else {
			output = new Data_1D(this->height, 0);
	...
	void Data_2D::Shift(Data_1D* profile, Direction direction, OverflowMode mode)
	{
		ScriptAssert(profile != NULL);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {		
				if (direction == GGEN_VERTICAL) {
					Height distance = profile->GetValueInterpolated(x, this->width);
					...
					Height distance = profile->GetValueInterpolated(y, this->height);
	...
	void Data_2D::GradientFromProfile(Coord from_x, Coord from_y, Coord to_x, Coord to_y, Data_1D* pattern, bool fill_outside)
	{
		ScriptAssert(pattern != NULL);
		ScriptAssert(from_x < this->width);
		ScriptAssert(from_y < this->height);
		ScriptAssert(to_x < this->width);
		ScriptAssert(to_y < this->height);

		ExtExtHeight target_x = to_x - from_x;
		ExtExtHeight target_y = to_y - from_y;
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				ExtExtHeight point_x = x - from_x;
				ExtExtHeight point_y = y - from_y;
				...
				ExtExtHeight cross_x = (target_x * (target_x * point_x + target_y * point_y)) / (target_x * target_x + target_y * target_y);
				ExtExtHeight cross_y = (target_y * (target_x * point_x + target_y * point_y)) / (target_x * target_x + target_y * target_y);		
				...
				if(distance <= max_dist && reverse_distance <= max_dist) {
					this->data[x + this->width * y] = pattern->GetValueInterpolated((Distance) distance, (Size) max_dist + 1);
	...
	void Data_2D::Gradient(Coord from_x, Coord from_y, Coord to_x, Coord to_y, Height from_value, Height to_value, bool fill_outside)
	{
		ScriptAssert(from_x < this->width);
		ScriptAssert(from_y < this->height);
		ScriptAssert(to_x < this->width);
	...
	void Data_2D::RadialGradientFromProfile(Coord center_x, Coord center_y, Distance radius, Data_1D* pattern, bool fill_outside)
	{
		ScriptAssert(pattern != NULL);
		ScriptAssert(center_x < this->width);
		ScriptAssert(center_y < this->height);

		ScriptAssert(radius > 0);
		ScriptAssert(pattern != NULL);

		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				Distance distance = (Distance) sqrt((double) (ABS(x - center_x) * ABS(x - center_x) + ABS(y - center_y) * ABS(y - center_y)));
	...
	void Data_2D::RadialGradient(Coord center_x, Coord center_y, Coord radius, Height min, Height max, bool fill_outside)
	{
		ScriptAssert(center_x < this->width);
		ScriptAssert(center_y < this->height);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				...
				if (distance < radius) {
					this->data[x + this->width * y] = (Height) ((ExtExtHeight) min + rel_max * (ExtExtHeight) distance / (ExtExtHeight) radius);
				} else if (fill_outside) {
					this->data[x + this->width * y] = (Height) ((ExtHeight) min + rel_max);
				}
			}
		}
	}
	...
	double Noise_GetGridPoint(Data_2D* map, CoordOffset x, CoordOffset y, Height* verticalOverflowBuffer, Height* horizontalOverflowBuffer){
	...
	Height Noise_BicubicInterpolation(double p[4][4]){
		...
		// Check for overflows.
		ScriptAssert(value >= GGEN_MIN_HEIGHT && value <= GGEN_MAX_HEIGHT);

		return (Height) value;
	}

	void Data_2D::Noise(Size minFeatureSize, Size maxFeatureSize, Amplitudes* amplitudes)
	{
		ScriptAssert(amplitudes != NULL);	
		ScriptAssert(minFeatureSize > 0);
		ScriptAssert(minFeatureSize <= Generator::GetMaxMapSize());
		ScriptAssert(maxFeatureSize > 0);
		ScriptAssert(maxFeatureSize <= Generator::GetMaxMapSize());
		...
		Height* verticalOverflowBuffer = new Height[this->height + 1];
		Height* horizontalOverflowBuffer = new Height[this->width];
		...
		Size waveLength = 1 << waveLengthLog2;
		Height amplitude = amplitudes->data[waveLengthLog2];

		// Generate the initial seed values (square corners).
		for(Coord y = 0; ; y += waveLength){
			if(y < this->height){
				for(Coord x = 0; ; x += waveLength){
					if(x < this->width){
						this->data[x + this->width * y] = Random<Height>(-amplitude, amplitude);
					}
					else{
						verticalOverflowBuffer[y] = Random<Height>(-amplitude, amplitude);
						break;
					}
				}
			}
				...
				for(Coord x = 0; ; x += waveLength){
					if(x < this->width){
						horizontalOverflowBuffer[x] = Random<Height>(-amplitude, amplitude);
					}
					else{
						verticalOverflowBuffer[this->height] = Random<Height>(-amplitude, amplitude);
						break;
					}
				}
				break;
		...
		while(waveLength > 1){
			Size halfWaveLength = waveLength / 2;

			// The square step - put a randomly generated point into center of each square.
			bool breakY = false;
			for(Coord y = halfWaveLength; breakY == false; y += waveLength){			
				for(Coord x = halfWaveLength; ; x += waveLength){
					// Prepare the 4x4 value matrix for bicubic interpolation.
					CoordOffset x0 = (int) x - (int) halfWaveLength - (int) waveLength;
					CoordOffset x1 = (int) x - (int) halfWaveLength;
					CoordOffset x2 = (int) x + (int) halfWaveLength;
					CoordOffset x3 = (int) x + (int) halfWaveLength + (int) waveLength;

					CoordOffset y0 = (int) y - (int) halfWaveLength - (int) waveLength;
					CoordOffset y1 = (int) y - (int) halfWaveLength;
					CoordOffset y2 = (int) y + (int) halfWaveLength;
					CoordOffset y3 = (int) y + (int) halfWaveLength + (int) waveLength;
					...
					// Interpolate!
					Height interpolatedHeight = Noise_BicubicInterpolation(data) + (waveLength < minFeatureSize ? 0 : Random<ExtHeight>(-amplitude, amplitude));
			...
			breakY = false;
			bool evenRow = true;
			for(Coord y = 0; breakY == false; y += halfWaveLength){			
				for(Coord x = evenRow ? halfWaveLength : 0; ; x += waveLength){							
					// Prepare the 4x4 value matrix for bicubic interpolation (this time rotated by 45 degrees).
					CoordOffset x0 = (int) x - (int) halfWaveLength - (int) waveLength;
					CoordOffset x1 = (int) x - (int) waveLength;
					CoordOffset x2 = (int) x - (int) halfWaveLength;
					CoordOffset x3 = (int) x;
					CoordOffset x4 = (int) x + (int) halfWaveLength;
					CoordOffset x5 = (int) x + (int) waveLength;
					CoordOffset x6 = (int) x + (int) halfWaveLength + (int) waveLength;

					CoordOffset y0 = (int) y - (int) halfWaveLength - (int) waveLength;
					CoordOffset y1 = (int) y - (int) waveLength;
					CoordOffset y2 = (int) y - (int) halfWaveLength;
					CoordOffset y3 = (int) y;
					CoordOffset y4 = (int) y + (int) halfWaveLength;
					CoordOffset y5 = (int) y + (int) waveLength;
					CoordOffset y6 = (int) y + (int) halfWaveLength + (int) waveLength;
			...
			waveLength /= 2;
			if(waveLength > 1){						
				amplitude = amplitudes->data[Log2(waveLength / 2)];
			}
		}
	...
	void Data_2D::VoronoiNoise(Size cell_size, uint8 points_per_cell, VoronoiNoiseMode mode)
	{
		ScriptAssert(cell_size > 2);
		ScriptAssert(points_per_cell >= 1);
		...
		ExtExtHeight max_dist = 2 * cell_width * cell_width + cell_height * cell_height;
		...
		for (int y = 0; y < num_cells_y; y++) {
			for (int x = 0; x < num_cells_x; x++) {
				for (int i = 0; i < points_per_cell; i++) {
					if (x == 0) {
						VORONOINOISE_GET_POINT(x, y, i).x = Random<int>(0, overlap_x);
					} else if (x < num_cells_x - 1) {
						VORONOINOISE_GET_POINT(x, y, i).x = overlap_x + (x - 1) * cell_width + Random<int>(0, cell_width);
					} else {
						VORONOINOISE_GET_POINT(x, y, i).x = overlap_x + (x - 1) * cell_width + Random<int>(0, overlap_x);
					}

					if (y == 0) {
						VORONOINOISE_GET_POINT(x, y, i).y = Random<int>(0, overlap_y);
					} else if (y < num_cells_y - 1) {
						VORONOINOISE_GET_POINT(x, y, i).y = overlap_y + (y - 1) * cell_height + Random<int>(0, cell_height);
					} else {
						VORONOINOISE_GET_POINT(x, y, i).y = overlap_y + (y - 1) * cell_height + Random<int>(0, overlap_y);
					}
		...
		for(Coord y = 0; y < height; y++){
			for(Coord x = 0; x < width; x++){
				...
				Coord cell_x = x / cell_width;
				Coord cell_y = y / cell_height;
				...
				ExtExtHeight current_min_dist = max_dist - 1;
				ExtExtHeight current_second_min_dist = max_dist - 1;
				...
				for(int i = 0; i < num_points_waiting; i++){
					for(int j = 0; j < points_per_cell; j++){
						...
						if(current_distance <= current_min_dist){
							...
							current_min_dist = (ExtExtHeight) current_distance;
						}
						...
						if(current_distance > current_min_dist && current_distance < current_second_min_dist){
							current_second_min_dist = (ExtExtHeight) current_distance;
						}
					}
				}
			
				/* Calculate value for current point from the distances to the two closest points */
				if(mode == GGEN_RIDGES) data[x + y * width] = (Height) ((-current_min_dist + current_second_min_dist) * GGEN_MAX_HEIGHT / max_dist);
				else if(mode == GGEN_BUBBLES) data[x + y * width] = (Height) (current_min_dist * GGEN_MAX_HEIGHT / max_dist);
			}
		}
	 ...
	 */
	void Data_2D::Flood(double water_amount)
	{
		ScriptAssert(water_amount < 1 && water_amount > 0);
	...
	void Data_2D::Smooth(Distance radius)
	{
		ScriptAssert(radius > 0);
	...
	void Data_2D::SmoothDirection(Distance radius, Direction direction)
	{
		ScriptAssert(radius > 0);
		...
		if (direction == GGEN_HORIZONTAL) {
			for (Coord y = 0; y < height; y++) {
				...
				ExtHeight window_value = this->data[this->width * y] * radius;

				for (Distance x = 0; x < radius; x++) {
			...
			for (Coord x = 0; x < this->width; x++) {
				...
				Size window_size = radius * 2 + 1;
				ExtHeight window_value = this->data[x] * radius;

				for (Distance y = 0; y < radius; y++) {
	...
	void Data_2D::Pattern(Data_2D* pattern)
	{
		ScriptAssert(pattern != NULL);
	...
	void Data_2D::ReturnAs(const String &name)
	{
		if(Generator::GetInstance()->return_callback == NULL) {
			Generator::GetInstance()->ThrowMessage(StaticString("The script returned a named map, but return handler was not defined"), GGEN_WARNING);
			return;
		}

		/* Call the defined return callback */
		Generator::GetInstance()->return_callback(name, this->data, this->width, this->height);
	}

	void Data_2D::Monochrome(Height threshold)
	{
		for (Index i = 0; i < this->length; i++) {
	...
	void Data_2D::SelectValue(Height value)
	{
		for (Index i = 0; i < this->length; i++) {
	...
	void Data_2D::SlopeMap()
	{
		/* Allocate the new array */
		Height* new_data = new Height[this->length];

		ScriptAssert(new_data != NULL);
		ScriptAssert(this->width > 2 && this->height > 2);
	...
	void Data_2D::Scatter(bool relative)
	{
		Height max = GGEN_UNRELATIVE_CAP;
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {		
				this->data[x + y * this->width] = Random<Height>(0, max) > this->data[x + y * this->width] ? 0 : 1;
			}
		}
	}

	void Data_2D::TransformValues(Data_1D* profile, bool relative)
	{
		ScriptAssert(profile != NULL);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {	
				if (this->data[x + y * this->width] > 0) {
					Height height = this->data[x + this->width * y];
	...
	void Data_2D::NormalizeDirection(Direction direction, NormalizationMode mode)
	{
		if (direction == GGEN_HORIZONTAL) {
			for (Coord y = 0; y < this->height; y++){
				Height last = this->data[y * this->width];
			...
			for (Coord y = this->height - 1; y > 0; y--) {
				Height last = this->data[y * this->width + this->width - 1];

				for (Coord x = this->width - 1; x > 0; x--) {
					if (mode == GGEN_ADDITIVE && this->data[x + y * this->width] > last + 1) {
						this->data[x + y * this->width] = last + 1;
					} else if (mode == GGEN_SUBSTRACTIVE && this->data[x + y * this->width] < last - 1) {
						this->data[x + y * this->width] = last - 1;
			...
			for (Coord x = this->width - 1; x > 0; x--) {
				Height last = this->data[this->length - (this->width - x)];
	...
	void Data_2D::Normalize(NormalizationMode mode) 
	{
		this->NormalizeDirection(GGEN_HORIZONTAL, mode);
		this->NormalizeDirection(GGEN_VERTICAL, mode);
	}

	void Data_2D::Transform(double a11, double a12, double a21, double a22, bool preserve_size)
	{
		/* The matrix must be invertible (its determinant must not be 0) */
		ScriptAssert(a11 * a22 - a12 * a21 != 0);
		...
		CoordOffset new_left_x = (int32) floor(MIN(MIN(0, new_top_right_x), MIN(new_bottom_left_x, new_bottom_right_x)));
		CoordOffset new_right_x = (int32) ceil(MAX(MAX(0, new_top_right_x), MAX(new_bottom_left_x, new_bottom_right_x)));
	
		CoordOffset new_top_y = (int32)floor(MIN(MIN(0, new_top_right_y), MIN(new_bottom_left_y, new_bottom_right_y)));
		CoordOffset new_bottom_y = (int32) ceil(MAX(MAX(0, new_top_right_y), MAX(new_bottom_left_y, new_bottom_right_y)));
	
		CoordOffset new_width = new_right_x - new_left_x + 1;
		CoordOffset new_height = new_bottom_y - new_top_y + 1;
	
		/* Make sure the output dimensions fit into a 16 bit unsigned integer, so we don't have array overflows later */
		ScriptAssert(new_width < 2 << 16 && new_height < 2 << 16);
		...
		if (preserve_size) {
			...
			/* Allocate the new array */
			new_data = new Height[this->length];
		} else { 
			...
			/* Allocate the new array */
			new_data = new Height[new_length];
		}
		...
		for (CoordOffset new_y = from_y; new_y < to_y; new_y++) {
			/* The second multiplication always stays the same for whole row */
			Index y_part_1 = (Height) ((new_y - new_origin_y) * inverted_a12);
			Index y_part_2 = (Height) ((new_y - new_origin_y) * inverted_a22);
		
			/* Offset from pointer from the first cell in the array to the first cell in current row */
			CoordOffset y_offset = (new_y - from_y) * new_width;
		
			for (CoordOffset new_x = from_x; new_x < to_x; new_x++) {
				...
				CoordOffset x = (CoordOffset) ((new_x - new_origin_x) * inverted_a11) + y_part_1;
				CoordOffset y = (CoordOffset) ((new_x - new_origin_x) * inverted_a21) + y_part_2;
	...
	void Data_2D::Rotate(int32 angle, bool preserve_size)
	{
	...
	void Data_2D::Shear(int32 horizontal_shear, int32 vertical_shear, bool preserve_size){
		/* Vertical and horizontal shear == 1 ==> the transformation matrix would be non-invertible */
		ScriptAssert(horizontal_shear != 1 || vertical_shear != 1);
	 ...
	 void Data_2D::Flip(Direction direction){
		this->Transform(
	...
	class FillPolygon_DownwardsEdge{
	public:
		double x; /* X of the upper (and in process current) point */
		CoordOffset y; /* Y of the upper point */
		CoordOffset dy; /* Edge height */
		...
		static bool CompareWithY(FillPolygon_DownwardsEdge* a, FillPolygon_DownwardsEdge* b){
			/* Sort first by Y */
		...
		static bool CompareWithoutY(FillPolygon_DownwardsEdge* a, FillPolygon_DownwardsEdge* b){
			/* Then by X */
			if (a->x < b->x) return true;
			else if (b->x < a->x) return false;
		...
		static bool IsRemoveable(FillPolygon_DownwardsEdge* victim){
			return victim->dy == 0;
		}
	};

	void Data_2D::FillPolygon(Path* path, Height value){
		ScriptAssert(path != NULL);

		/* Create a full list of all non-horizontal edges (this is line algorithm, we can skip horizontal edges) */
		list<FillPolygon_DownwardsEdge*> edges;
		for (Path::Iterator i = path->points.begin(); i != path->points.end();) {
			...
			FillPolygon_DownwardsEdge* edge = new FillPolygon_DownwardsEdge();
		...
		edges.sort(FillPolygon_DownwardsEdge::CompareWithY);

		/* Edges intersecting current line */
		list<FillPolygon_DownwardsEdge*> currentLineEdges;
		bool needsSorting = true;

		/* For each line from the top-most point to the bottom edge of the map */
		for (CoordOffset y = (*edges.begin())->y; y < this->height; y++) {

			/* Add edges starting on this line to the currently worked list */
			list<FillPolygon_DownwardsEdge*>::iterator i = edges.begin();
			...
			if (needsSorting) {
				currentLineEdges.sort(FillPolygon_DownwardsEdge::CompareWithoutY);
			...
			for (list<FillPolygon_DownwardsEdge*>::iterator j = currentLineEdges.begin(); j != currentLineEdges.end(); j++) {
				FillPolygon_DownwardsEdge* edge = *j;
			...
			currentLineEdges.remove_if(FillPolygon_DownwardsEdge::IsRemoveable);
		}	
	}

	/* Helper data structure for the following algorithm. */
	struct StrokePath_Quad{
		Coord x, y;
		Size width, height;

		StrokePath_Quad(Coord x, Coord y, Size width, Size height):x(x), y(y), width(width), height(height){
		}
	};


	/* Helper function used to calculated distance from one map point to one path segment defined by two points. */
	Height GGen_StrokePath_GetDistanceToSegment(Point segmentPoint1, Point segmentPoint2, Point mapPoint){
		...
		if(det < 0 || det > length)
		{
			...
			currentDistance = (Distance) (sqrt(MIN(vx * vx + vy * vy, ux * ux + uy * uy)) + 0.5);						
		}
		else {
			det = ux * vy - uy * vx;
			currentDistance = (Distance) (sqrt((det * det) / length) + 0.5);
		}

		return currentDistance;
	}

	void Data_2D::StrokePath(Path* path, Data_1D* brush, Distance radius, bool fill_outside) {
		ScriptAssert(path != NULL);
		ScriptAssert(brush != NULL);
		ScriptAssert(radius > 0);	
		...
		vector<Point> segmentArray = vector<Point>(path->points.begin(), path->points.end());

		/* Distance from each tile to the nearest path segment. Tiles, that were not calculated yet hold GGEN_INVALID_HEIGHT. */
		Height* segmentDistances = new Height[this->length];
		 ...
		 * stack overflows. */
		stack<StrokePath_Quad> quadStack;

		/* First step will be on whole map. */
		quadStack.push(StrokePath_Quad(0, 0, this->width, this->height));

		/* Initiate the "recursion". */
		while(!quadStack.empty()){
			StrokePath_Quad currentQuad = quadStack.top();
			quadStack.pop();		
			...
			Point cornerPoints[4] = 
				{
					Point(currentQuad.x, currentQuad.y), // top-left
					Point(currentQuad.x + currentQuad.width - 1, currentQuad.y), // top-right
					Point(currentQuad.x, currentQuad.y + currentQuad.height - 1), // bottom-left
					Point(currentQuad.x + currentQuad.width - 1, currentQuad.y + currentQuad.height - 1), // bottom-right
				};
			...
			for(int cornerNumber = 0; cornerNumber < 4; cornerNumber++){
				Point currentPoint = cornerPoints[cornerNumber];
				Index currentIndex = currentPoint.x + this->width * currentPoint.y;
			
				if(segmentDistances[currentIndex] == GGEN_INVALID_HEIGHT){				
					/* Calculate distance to the closest path segment */
					Distance distance = this->GetMaxDistance();
					...
					for(Path::Iterator i = path->points.begin(); i != path->points.end(); ){
						...
						Distance currentDistance = GGen_StrokePath_GetDistanceToSegment(point1, point2, currentPoint);
			...
			if(!isSubdivisionNeeded){
				...
				Point point1 = segmentArray[currentSegmentIndex];
				Point point2 = segmentArray[currentSegmentIndex + 1];

				for(CoordOffset offsetY = 0; offsetY < currentQuad.height; offsetY++){
					for(CoordOffset offsetX = 0; offsetX < currentQuad.width; offsetX++){
						Coord x = currentQuad.x + offsetX;
						Coord y = currentQuad.y + offsetY;
					
						Distance currentDistance = GGen_StrokePath_GetDistanceToSegment(point1, point2, Point(x, y));
			...
			if(currentQuad.width > 1 || currentQuad.height > 1){
				...
				Size subdidedQuadWidth = currentQuad.width > 1 ? currentQuad.width / 2 : 1;
				Size subdidedQuadHeight = currentQuad.height > 1 ? currentQuad.height / 2 : 1;

				/* Subdivide horizontally and/or vertically (don't subdivide if only one tile in that direction is remaining). */
				quadStack.push(StrokePath_Quad(currentQuad.x, currentQuad.y, subdidedQuadWidth, subdidedQuadHeight));
			
				if(currentQuad.width > 1){
					quadStack.push(StrokePath_Quad(currentQuad.x + subdidedQuadWidth, currentQuad.y, currentQuad.width - subdidedQuadWidth, subdidedQuadHeight));
				}
			
				if(currentQuad.height > 1){
					quadStack.push(StrokePath_Quad(currentQuad.x, currentQuad.y + subdidedQuadHeight, subdidedQuadWidth, currentQuad.height - subdidedQuadHeight));
					if(currentQuad.width > 1){
						quadStack.push(StrokePath_Quad(currentQuad.x + subdidedQuadWidth, currentQuad.y + subdidedQuadHeight, currentQuad.width - subdidedQuadWidth, currentQuad.height - subdidedQuadHeight));
					}
				}
			}
		}
		...
		for(Coord y = 0; y < this->height; y++){
			for(Coord x = 0; x < this->width; x++){
				Distance currentDistance = segmentDistances[x + this->width * y];
	...
	void Data_2D::FloodFillBase(Coord start_x, Coord start_y, Height fill_value, ComparisonMode mode, Height threshold, bool select_only){
		ScriptAssert(start_x < this->width);
		ScriptAssert(start_y < this->height);		
		...
		while (!queue.empty()) {
			...
			Height currentValue = this->data[current.x + current.y * this->width];
	...
	void Data_2D::FloodFill(Coord start_x, Coord start_y, Height fill_value, ComparisonMode mode, Height threshold){
		ScriptAssert(start_x < this->width);
		ScriptAssert(start_y < this->height);		
	...
	void Data_2D::FloodSelect(Coord start_x, Coord start_y, ComparisonMode mode, Height threshold){
		ScriptAssert(start_x < this->width);
		ScriptAssert(start_y < this->height);		
	...
	Height Data_2D::GetValueOnPathBase(Path* path, bool max){
		ScriptAssert(path != NULL);
		ScriptAssert(path->points.size() > 1);

		Height extreme = max ? GGEN_MIN_HEIGHT : GGEN_MAX_HEIGHT;
		...
		for (Path::Iterator i = path->points.begin(); i != path->points.end();) {
			...
			if(ABS(currentPoint->x - nextPoint->x) > ABS(currentPoint->y - nextPoint->y)){
				...
				if (currentPoint->GetX() > nextPoint->GetX()) {
					//swap<Point*>(currentPoint, nextPoint);
					Point* tempPoint = currentPoint;
					currentPoint = nextPoint;
					nextPoint = tempPoint;
				}
				...
				CoordOffset x = currentPoint->x;	

				/* The first point */
				if(x > 0 && x < this->width && y > 0 && y < height){
					/* Y coordinate must be rounded */
					Height current = this->data[x + (CoordOffset) (y + 0.5) * this->width]; 
				...
				while(x < nextPoint->x){
					...
					if(x > 0 && x < this->width && (CoordOffset) y > 0 && (CoordOffset) y < height){
						/* Y coordinate must be rounded */
						Height current = this->data[x + (CoordOffset) (y + 0.5) * this->width]; 
				...
				CoordOffset y = currentPoint->y;	

				/* The first point */
				if(x > 0 && x < this->width && y > 0 && y < height){
					/* X coordinate must be rounded */
					Height current = this->data[(CoordOffset) (x + 0.5) + y * this->width]; 
	...
	Height Data_2D::GetMaxValueOnPath(Path* path){
		ScriptAssert(path != NULL);
		ScriptAssert(path->points.size() > 1);
	...
	Height Data_2D::GetMinValueOnPath(Path* path){
		ScriptAssert(path != NULL);
		ScriptAssert(path->points.size() > 1);
	...
	void Data_2D::ExpandShrinkDirectionBase(Distance distance, Direction direction, bool shrink){
		ScriptAssert(distance > 0);
		ScriptAssert(direction == GGEN_VERTICAL || distance < this->width);
		ScriptAssert(direction == GGEN_HORIZONTAL || distance < this->height);
		...
		if (direction == GGEN_HORIZONTAL) {
			for (Coord y = 0; y < height; y++) {
				/* Prefill the window with value of the left edge + n leftmost values (where n is radius) */
				Size window_size = distance * 2 + 1;
				Height window_value = (Height) (this->data[this->width * y] > 0) * distance;

				for (Distance x = 0; x < distance; x++) {
					window_value += (Height) (this->data[x + this->width * y] > 0);
				}
				...
				for (Coord x = 0; x < this->width; x++) {
					/* If the window is approaching right border, use the rightmost value as fill. */
					if (x < distance) {
						window_value += (Height) (this->data[x + distance + this->width * y] > 0) - (Height) (this->data[this->width * y] > 0);
					} else if (x + distance < this->width) {
						window_value += (Height) (this->data[x + distance + this->width * y] > 0) - (Height) (this->data[x - distance + this->width * y] > 0);
					} else {
						window_value += (Height) (this->data[this->width - 1 + this->width * y] > 0) - (Height) (this->data[x - distance + this->width * y] > 0);
					}

					/* Set the value of current tile to 1 as long as there is at least one value greater than zero in the current window. */
					new_data[x + this->width * y] = (Height) (window_value > 0) * (shrink ? -1 : 1);
				}
			}
		} else { /* vertical */
			for (Coord x = 0; x < this->width; x++) {
				/* Prefill the window with value of the left edge + n topmost values (where n is radius) */
				Size window_size = distance * 2 + 1;
				Height window_value = (Height) (this->data[x] > 0) * distance;

				for (Distance y = 0; y < distance; y++) {
					window_value += (Height) (this->data[x + y * this->width] > 0);
				}
				...
				for (Coord y = 0; y < this->height; y++) {
					/* If the window is approaching right border, use the rightmost value as fill. */
					if (y < distance) {
						window_value += (Height) (this->data[x + (y + distance) * this->width] > 0) - (Height) (this->data[x] > 0);
					} else if (y + distance < height) {
						window_value += (Height) (this->data[x + (y + distance) * this->width] > 0) - (Height) (this->data[x + (y - distance) * this->width] > 0);
					} else {
						window_value += (Height) (this->data[x + (this->height - 1) * this->width] > 0) - (Height) (this->data[x + (y - distance) * this->width] > 0);
					}
	...
	void Data_2D::ExpandDirection(Distance distance, Direction direction){
		ScriptAssert(distance > 0);
		ScriptAssert(direction == GGEN_VERTICAL || distance < this->width);
		ScriptAssert(direction == GGEN_HORIZONTAL || distance < this->height);
	...
	void Data_2D::ShrinkDirection(Distance distance, Direction direction){
		ScriptAssert(distance > 0);
		ScriptAssert(direction == GGEN_VERTICAL || distance < this->width);
		ScriptAssert(direction == GGEN_HORIZONTAL || distance < this->height);
	...
	void Data_2D::Expand(Distance distance){
		ScriptAssert(distance > 0);
		ScriptAssert(distance < this->width);
		ScriptAssert(distance < this->height);

		this->ExpandDirection(distance, GGEN_HORIZONTAL);
		this->ExpandDirection(distance, GGEN_VERTICAL);
	}

	void Data_2D::Shrink(Distance distance){
		ScriptAssert(distance > 0);
		ScriptAssert(distance < this->width);
		ScriptAssert(distance < this->height);
	...
	void Data_2D::Outline(ComparisonMode mode, Height threshold, OutlineMode outlineMode)
	{
		...
		for(Coord y = 0; y < this->height; y++){
			...
			Index yIndexOffset = y * this->width;

			/* A cell is part of a border, if it matches the condition and at least one of its neighbors doesn't. */
			for(Coord x = 1; x < this->width - 1; x++){
				Height prevValue = this->data[x - 1 + yIndexOffset];
				Height currentValue = this->data[x + yIndexOffset];
				Height nextValue = this->data[x + 1 + yIndexOffset];

				switch(mode){
					case GGEN_EQUAL_TO: currentValue = (Height) ((currentValue == threshold) && (!(nextValue == threshold) || !(prevValue == threshold))); break;
					case GGEN_NOT_EQUAL_TO: currentValue = (Height) ((currentValue != threshold) && (!(nextValue != threshold) || !(prevValue != threshold))); break;
					case GGEN_GREATER_THAN: currentValue = (Height) ((currentValue > threshold) && (!(nextValue > threshold) || !(prevValue > threshold))); break;
					case GGEN_GREATER_THAN_OR_EQUAL_TO: currentValue = (Height) ((currentValue >= threshold) && (!(nextValue >= threshold) || !(prevValue >= threshold))); break;
					case GGEN_LESS_THAN: currentValue = (Height) ((currentValue < threshold) && (!(nextValue < threshold) || !(prevValue < threshold))); break;
					case GGEN_LESS_THAN_OR_EQUAL_TO: currentValue = (Height) ((currentValue <= threshold) && (!(nextValue <= threshold) || !(prevValue <= threshold))); break;
				}
		...
		for(Coord x = 0; x < this->width; x++){
			...
			/* A cell is part of a border, if it matches the condition and at least one of its neighbors doesn't. */
			for(Coord y = 1; y < this->height - 1; y++){
				Height prevValue = this->data[x + (y - 1) * this->width];
				Height currentValue = this->data[x + y * this->width];
				Height nextValue = this->data[x + (y + 1) * this->width];
	...
	void Data_2D::ConvexityMap(Distance radius)
	{
		ScriptAssert(radius > 0);
	...
	void Data_2D::Distort(Size waveLength, Distance amplitude)
	{
		ScriptAssert(waveLength > 0);
		ScriptAssert(waveLength < this->width);
		ScriptAssert(waveLength < this->height);
		ScriptAssert(amplitude > 0);
		ScriptAssert(amplitude < this->width);
		ScriptAssert(amplitude < this->height);

		/* Set up an Amplitude object with one wave length only. */
		Amplitudes* amplitudeObject = new Amplitudes(waveLength);
		...
		Data_2D* turbulenceXMap = new Data_2D(this->width, this->height, 0);
		turbulenceXMap->Noise(1, waveLength, amplitudeObject);

		Data_2D* turbulenceYMap = new Data_2D(this->width, this->height, 0);
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				Coord distortedX = MAX(MIN((CoordOffset) x + (CoordOffset) turbulenceXMap->data[x + y * this->width], (CoordOffset) (this->width - 1)), 0);
				Coord distortedY = MAX(MIN((CoordOffset) y + (CoordOffset) turbulenceYMap->data[x + y * this->width], (CoordOffset) (this->height - 1)), 0);
	...
	void Data_2D::NormalDifferenceMap(int32 angle){
		/* Clamp the angle to the 0-360 range */
		angle = angle % 360;
		...
		for (Coord y = 0; y < this->height; y++) {
			for (Coord x = 0; x < this->width; x++) {
				...
				Height normal = this->GetNormal(x, y);
				...
				ExtHeight normalDifference = ABS(((ExtHeight) normal - (ExtHeight) angle));
	...
	Height Data_2D::GetNormal( Coord x, Coord y )
	{
		ScriptAssert(x < this->width);
		ScriptAssert(y < this->height);

		Index indexLeft = x > 0 ? (x - 1) + y * this->width : x + y * this->width;
		Index indexRight = x < this->width ? (x + 1) + y * this->width : x + y * this->width;
		Index indexTop = y > 0 ? x + (y - 1) * this->width : x + y * this->width;
		Index indexBottom = y < this->height ? x + (y + 1) * this->width : x + y * this->width;

		Height heightLeft = this->data[indexLeft];
		Height heightRight = this->data[indexRight];
		Height heightTop = this->data[indexTop];
		Height heightBottom = this->data[indexBottom];
		...
		return (Height) (angle / 3.14159 * GGEN_MAX_HEIGHT);
	}

	void Data_2D::SimpleErosion(uint8 numRounds, uint8 erosionFactor, bool enableSedimentation)
	{
		//ScriptAssert(this->width == flowMap->width && this->height == flowMap->height);
		//ScriptAssert(this->width == sedimentMap->width && this->height == sedimentMap->height);

		// Each round is completely separate.
		for(uint32 round = 0; round < numRounds * this->length; round++){
			// Choose a random tile on the map.
			Coord x = Random<Coord>(0, this->width - 1);
			Coord y = Random<Coord>(0, this->height - 1);
		
			Height currentCarriedSediment = 0;
			...
			while(this->data[x + this->width * y] > 0){
				...
				Coord lowestNeighborX = x;
				Coord lowestNeighborY = y;
				Height lowestNeighborHeight = this->data[x + this->width * y];

				// Try to look at 5 random points in the neighborhood (to add a little randomness into the flow).
				for(uint8 i = 0; i < 5; i++){		
					CoordOffset currentNeighborX = (CoordOffset) x + Random<int8>(-1, 1);
					CoordOffset currentNeighborY = (CoordOffset) y + Random<int8>(-1, 1);
