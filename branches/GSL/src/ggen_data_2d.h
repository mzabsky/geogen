...
namespace GeoGen{
	...
	class Data_2D{
		...
		public:
			...
			/**
			 * Creates new Data_2D object of given size.
			 * @param width Width of the map.
			 * @param height Height of the map.
			 * @param value Default value for all cells.
			 **/
			Data_2D(Size width, Size height, Height value);
			 ...
			 **/
			void SetValue(Coord x, Coord y, Height value);
			 ...
			 * @param value Value to use.
			 **/
			void SetValueInRect(Coord x1, Coord y1, Coord x2, Coord y2, Height value);
			 ...
			 **/
			Height GetValue(Coord x, Coord y);
			 ...
			 **/
			void AddTo(Data_2D* addend, CoordOffset offset_x, CoordOffset offset_y);
			 ...
			 **/
			void AddMapMasked(Data_2D* addend, Data_2D* mask, bool relative);
			 ...
			 * @note 0 in the mask always means the original value won't be changed. In relative mode, maximum value found in the mask then means 100%, otherwise 32767 means 100%.
			 **/
			void AddMasked(Height value, Data_2D* mask, bool relative);
			 ...
			 **/
			void MultiplyMap(Data_2D* factor);
			 ...
			 * @note If the new size is larger than the original, the new values will be calculated using the linear interpolation algorithm. Otherwise, the new values will be chosen using the nearest neighbor algorithm.
			 **/
			void ScaleTo(Size new_width, Size new_height, bool scale_values);
			 ...
			 **/
			void ScaleValuesTo(Height new_min, Height new_max);
			 ...
			 **/
			void ResizeCanvas(Size new_width, Size new_height, CoordOffset new_zero_x, CoordOffset new_zero_y);
		
			/**
			 * Sets all values in the map.
			 * @param value The fill value.
			 **/
			void Fill(Height value);
			 ...
			 * @note 0 in the mask always means the original value won't be changed. In relative mode, maximum value found in the mask then means 100%, otherwise 32767 means 100%.
			 **/
			void FillMasked(Height value, Data_2D* mask, bool relative);
			 ...
			 **/
			void Clamp(Height min, Height max);
			 ...
			 * @param max New maximum value.
			 **/
			void CropValues(Height min, Height max);
			 ...
			 **/
			void Intersection(Data_2D* victim);
			 ...
			 **/
			void IntersectionTo(Data_2D* victim, CoordOffset offset_x, CoordOffset offset_y);
			 ...
			 * @param offset_y Coordinates of the intersection map will be shifted by this value along the Y axis.
			 **/
			void UnionTo(Data_2D* victim, CoordOffset offset_x, CoordOffset offset_y);
			 ...
			 **/
			void Combine(Data_2D* victim, Data_2D* mask, bool relative);
			 ...
			 **/
			void ReplaceValue(Height needle, Height replace);
			 ...
			 * @param mode The overflow mode (see OverflowMode).
			 **/
			void Shift(Data_1D* profile, Direction direction, OverflowMode mode);
			 ...
			 **/
			void Project(Data_1D* profile, Direction direction);
			 ...
			 **/
			Data_1D* GetProfile(Direction direction, Coord coordinate);
			 ...
			 **/
			void Gradient(Coord from_x, Coord from_y, Coord to_x, Coord to_y, Height from_value, Height to_value, bool fill_outside);
			 ...
			 **/
			void GradientFromProfile(Coord from_x, Coord from_y, Coord to_x, Coord to_y, Data_1D* profile, bool fill_outside);
			 ...
			 **/
			void RadialGradient(Coord center_x, Coord center_y, Coord radius, Height from_value, Height to_value, bool fill_outside);
			 ...
			 **/
			void RadialGradientFromProfile(Coord center_x, Coord center_y, Distance radius, Data_1D* profile, bool fill_outside);
			 ...
			 * @param amplitudes Amplitudes object.
			 **/
			void Noise(Size min_feature_size, Size max_feature_size, Amplitudes* amplitudes);
			 ...
			 * @param mode The noise mode (see VoronoiNoiseMode).
			 ...
			 **/
			void VoronoiNoise(Size cell_size, uint8 points_per_cell, VoronoiNoiseMode mode);
			 ...
			 **/
			void SmoothDirection(Distance radius, Direction direction);
		
			/**
			 * Blurs the map. Uses linear smoothing algorithm.
			 * @param radius The smoothing kernel radius.
			 **/
			void Smooth(Distance radius);
			 ...
			 * @param mode The normalization mode (see NormalizationMode).
			 **/
			void NormalizeDirection(Direction direction, NormalizationMode mode);
			 ...
			 **/
			void ReturnAs(const String& label);
			 ...
			 **/
			void FillPolygon(Path* path, Height value);
			 ...
			 **/
			void StrokePath(Path* path, Data_1D* brush, Distance radius, bool fill_outside);

			void FloodFillBase(Coord start_x, Coord start_y, Height fill_value, ComparisonMode mode, Height threshold, bool select_only);	
			 ...
			 **/
			void FloodFill(Coord start_x, Coord start_y, Height fill_value, ComparisonMode mode, Height threshold);
			 ...
			 * @note The filling is 4-directional (the spread is only along the main two axes).
			 **/
			void FloodSelect(Coord start_x, Coord start_y, ComparisonMode mode, Height threshold);

			Height GetValueOnPathBase(Path* path, bool max);
			 ...
			 **/
			Height GetMaxValueOnPath(Path* path);
			 ...
			 **/
			Height GetMinValueOnPath(Path* path);

			void ExpandShrinkDirectionBase(Distance distance, Direction direction, bool shrink);
			 ...
			 **/
			void ExpandDirection(Distance distance, Direction direction);
			 ...
			 **/
			void ShrinkDirection(Distance distance, Direction direction);
			 ...
			 * @param distance The distance in maximum metric.
			 **/
			void Expand(Distance distance);

			/**
			 * Fills all areas within a distance (in maximum metric) from any negative value with 0. The rest of the map will be filled with 1.
			 * @param distance The distance in maximum metric.
			 **/
			void Shrink(Distance distance);
			 ...
			 **/
			void Outline(ComparisonMode mode, Height threshold, OutlineMode outlineMode);

			/**
			 * Replaces values in the array with information about steepness of slope (change in value) in that particular value.
			 * @param radius Convexity measurement radius (how big a terrain feature must be to show on the convexity map).
			 **/
			void ConvexityMap(Distance radius);
			 ...
			 **/
			void Distort(Size waveLength, Distance amplitude);
			 ...
			 **/
			Height GetNormal(Coord x, Coord y);	
			...
			static void FreeAllInstances(){
				while(Data_2D::instances.begin() != Data_2D::instances.end()){
					delete (*Data_2D::instances.begin());
				}
			};
	};
}
