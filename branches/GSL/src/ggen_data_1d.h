...
/** 
 * @file ggen_data_1d.h Data_1D represents a simple 1D array of values. It can be used to create more complex 2D maps using methods like GGen_Data_2D::Project, GGen_Data_2D::Gradient or GGen_Data_2D::TransformValues.
 **/
...
namespace GeoGen {
	/**
	 * Data_1D represents a simple 1D array of values. It can be used to create more complex 2D maps using methods like GGen_Data_2D::Project, GGen_Data_2D::Gradient or GGen_Data_2D::TransformValues.
	 **/
	class Data_1D{
		public:
			Height* data;
			...
			/**
			 * Creates new Data_1D object of given length.
			 * @param length Length of the array.
			 * @param value Default value for all cells.
			 **/
			Data_1D(Size length, Height value);
			 ...
			 **/
			void SetValue(Coord x, Height value);
			 ...
			 * @param value Value to use.
			 **/
			void SetValueInRange(Coord from, Coord to, Height value);
			 ...
			 **/
			Height GetValueInterpolated(Coord x, Size scale_to_length);
			 ...
			 **/
			void AddTo(Data_1D* addend, CoordOffset offset);
			 ...
			 **/		
			void AddMasked(Height value, Data_1D* mask, bool relative);
			 ...
			 * @note 0 in the mask always means the original value won't be changed. In relative mode, maximum value found in the mask then means 100%, otherwise 32767 means 100%.
			 **/	
			void AddArrayMasked(Data_1D* addend, Data_1D* mask, bool relative);
			 ...
			 **/
			void MultiplyArray(Data_1D* factor);
			 ...
			 * @note If the new size is larger than the original, the new values will be calculated using the linear interpolation algorithm. Otherwise, the new values will be chosen using the nearest neighbor algorithm.
			 **/
			void ScaleTo(Size new_length, bool scale_values);
			 ...
			 **/
			void ResizeCanvas(Size new_length, CoordOffset new_zero);
			 ...
			 * @param overflow_mode Overflow mode (see OverflowMode).
 			 * @note All values outside the array will be discarded. Newly created values will be set to 0.
			 **/
			void Shift(CoordOffset offset, OverflowMode overflow_mode);
			 ...
			 **/
			void Intersection(Data_1D* victim);
			 ...
			 **/
			void Gradient(Coord from, Coord to, Height from_value, Height to_value, bool fill_outside);
			...
			/**
			 * Creates a Path object from current object.
			 ...
			 **/
			Path* ToPath(uint16 point_count);
	};
}