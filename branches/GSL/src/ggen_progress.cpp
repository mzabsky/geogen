...
namespace GeoGen{
	void InitProgress(uint32 max_progress){
		// show a script assert if we are in wrong script mode, ordinary assert otherwise
		if(Generator::GetInstance()->GetStatus() == GGEN_LOADING_MAP_INFO) {ScriptAssert(Generator::GetInstance()->GetStatus() == GGEN_GENERATING);}
	...
	void SetProgress(uint32 current_progress){
		// show a script assert if we are in wrong script mode, ordinary assert otherwise
		if(Generator::GetInstance()->GetStatus() == GGEN_LOADING_MAP_INFO) {ScriptAssert(Generator::GetInstance()->GetStatus() == GGEN_GENERATING);}
		else assert(Generator::GetInstance()->GetStatus() == GGEN_GENERATING);

		ScriptAssert(current_progress <= Generator::GetInstance()->max_progress);
	...
	void IncreaseProgress(){
		...
		ScriptAssert(Generator::GetInstance()->current_progress + 1 < Generator::GetInstance()->max_progress);
